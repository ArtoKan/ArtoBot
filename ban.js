...
module.exports = {
    ...
    async run (bot, message, args) {
        ...
        if (!args[0]) return message.channel.send("Vous devez spécifier un utilisateur à bannir ! Essayez comme ça a!ban @user");

        if(!mentionMember) return message.channel.send("Cet utilisateur n'est pas un utilisateur valide / n'est plus sur le seveur !");
        ...
        await mentionMember.ban({
            reason: reason
        }).then(() => message.channel.send("Cette personne a bien été ban hehe: " + mentionMember.user.tag));
    }
}
